...
Many exchanges create a separate Google Sheets spreadsheet which lists
all of the treatless individuals and may or may not link to their requests. The
AutoAO3 App which we're using for our requests spreadsheet allows a lot of cool
sorting and filtering tricks, and we're using that to compile our treatless
list.

Once someone has received a treat, the mods will add an `Additional Tag`
which says **Treat Received**.

To find Treatless participants you only have to filter the request list
for those people who do NOT have that tag by using `!"Treat Received"`in the
...
1. Go to this year's Collection and click the `Post to Collection` button
   <u>or</u> go to `New Work`.
2. In the **Associations** section of the New Work make sure this year's collection is listed in the `Post to Collections / Challenges`
      box.
